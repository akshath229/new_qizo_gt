...
import 'Local_Db/Local_db_Model/LD_Model_Godown.dart';
import 'Local_Db/Local_db_Model/LD_Model_ItemMaster.dart';
import 'Local_Db/Local_db_Model/LD_Model_Sales.dart';
import 'Local_Db/Local_db_Model/LD_model_UnitTyp.dart';
import 'Local_Db/Local_db_Model/Ld_Model_LedgerHeades.dart';
...
class Newsalespage extends StatefulWidget {
  ...
  int passvalue;
  dynamic passname;
  Newsalespage({required this.passvalue,this.passname,this.itemrowdata});
...
//   befortax=taxOneItm*double.parse(quantityController.text);
  ...
  Future<bool> _onBackPressed() async {
    ...
    // Returning true indicates that the back press was handled
    return true;
  }
...
//
                      ...
                      onTap: () async {
                        ...
                        DateTime? date = await showDatePicker(
                            ...
                            builder: (BuildContext context, Widget? child) {   // Use Widget? for child
                              return Theme(
                                data: ThemeData.light(),
                                child: child!,   // Use the null assertion operator
                              );
                            }
                        );
                        if (date != null) {
...
//                 ),
  ...
  databinding(id) async{
    ...
    try {
      final bindata =
      await http.get("${Env.baseUrl}SalesHeaders/$id" as Uri, headers: { //Soheader //SalesHeaders
        "Authorization": user.user["token"],
      });
...
//---data binding old for salesorder-----
  ...
  databindingSoh(id) async{
    ...
    try {
      final bindata =
      await http.get("${Env.baseUrl}Soheader/$id/3" as Uri, headers: {
        "Authorization": user.user["token"],
      });
