...
//------------------for appbar------------
  ...
  GetdataPrint(id) async {
    ...
    try {
      final tagsJson =
      await http.get("${Env.baseUrl}SalesHeaders/57" as Uri, headers: {
        //Soheader //SalesHeaders
        "Authorization": user.user["token"],
      });
...
//
  ...
  Future<Uint8List> SetToImmg() async {
    ...
    // If the loop doesn't return, throw an exception or return a default value
    throw Exception("Failed to generate the image");
  }



  late Future<File> FinalImage;
