...
class NumberToWord {
  ...
  ///convert to Riyals
  NumberInRiyals(number,currency,paisa){
print("NumberInRiyals");
    var Num=number.runtimeType==double?number:double.parse(number);



    var s= Num.toString().split('.');
    var num1=convert(Locale.en_ind, int.parse(s[0].trim()));
    var num2=convert(Locale.en_ind, int.parse(s[1].trim()));


    var FinaString="";
    if(num1==""){
      FinaString="$num2 $paisa";
    }
    else if(num2==""){

      FinaString= "$num1 $currency";

    }else{
      FinaString="$num1 $currency and $num2 $paisa";
    }
